# -*- coding: utf-8 -*-
"""AI-Fianl-Project- Finetuning Language Models.ipynb
...
"""#The below model needs to be corrected as there is class imbalance: a complete pipeline for fine-tuning a DistilBERT model on a subset of patent applications
...
**Despite the efforts to train the model, the results indicate persistent issues with overfitting and class imbalance, as evidenced by the minimal improvement in validation loss and accuracy over the epochs.**

 The training loss remains nearly constant, while the validation loss does not show significant improvement, suggesting that the model struggles to generalize beyond the training data. The relatively low and stagnant accuracy further highlights the need for more advanced techniques, such as oversampling, learning rate scheduling, early stopping, and addressing class imbalances, which were not adequately implemented in this training process. These shortcomings prevent the model from achieving better performance and robust generalization.

 **Additionally, when I deployed the patent app on Hugging Face Spaces, I noticed that for all patent IDs, the patentability score consistently showed as PENDING, indicating a potential flaw in the prediction mechanism.**

# The below model is corrected finetuned model: Correcting class imbalance; Oversampling method
...
**Final Accuracy:** Maintaining an accuracy of around 43% is a strong indication that the model has effectively learned from the training data and is performing reasonably well on the validation data
...
**Balanced Approach:** By incorporating techniques like learning rate scheduling, weight decay, and data augmentation, I've created a balanced training regime that helps in preventing overfitting and encourages generalization.

**NOTE: oversampling process might have inadvertently resulted in only three classes being retained in your training set. This can happen if the original dataset has very few samples for the other classes, causing them to be excluded during the resampling process.**
...
"""#Connecting to Hugging Face spaces, push the model to it"""
